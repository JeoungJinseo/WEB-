...
1440×1024 디자인 영역을 화면 너비와 높이 안에 비율 그대로 맞춥니다. 인물은 확대해서 자르지 않으며 남는 영역은 영상 가장자리의 붉은 배경으로 채웁니다. 세로 화면에서는 메뉴와 설명을 다시 배치하고, 짧은 화면에서는 설명 영역을 스크롤해 모든 내용을 읽을 수 있습니다. 모션 감소 설정에서는 첨부 스틸 이미지로 전환합니다.
