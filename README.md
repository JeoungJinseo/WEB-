...
색감 차이를 줄이기 위해 스크롤용 MP4에 BT.709 색상 메타데이터를 명시했습니다. 픽셀을 재인코딩하지 않은 remux이며 원본 MP4는 그대로 보존합니다. PNG의 색상 테이블도 같은 BT.709 디코딩 기준으로 계산하고, 영상과 사진 모두 같은 암부·명암·디더링 합성 단계를 거칩니다. 수증기는 기존 농도를 유지합니다. 최종 대기 화면은 원본의 세밀한 질감을 사용하되 색감은 영상에 맞춥니다.
...
- `tests/scene-registration.test.mjs`: 연결 데이터와 원본 해시·정지 프레임의 일치, 배경 색 오차(4/255 이내), 암부 대비 보존, 파일 오류·취소 처리.
