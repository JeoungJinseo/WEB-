...
Portrait phones use a shared type and spacing unit based on a 390 px reference width. The collaboration mark, hero wordmark, introduction, supporting labels and credits retain their size ratios across phone widths. The introduction keeps the desktop's two lines per paragraph, with a smaller 10.5-unit type size and 1.3 line height. The profile keeps its original four-line title and right-hand discipline column. Copy and credits share a vertical anchor; short viewports move the group up without independently enlarging or reflowing its type. Landscape phones use a bounded content area and a contained wordmark.

The portrait character also scales from width, so a taller browser viewport no longer enlarges only the figure. Native footage, held frames and the compositor share this placement. Desktop/tablet geometry, media, palette, original desktop line breaks and motion order are unchanged; scene playback still takes 2.5 seconds. Compact screens retain the existing compatible clip with the same timing. The idle scene control stays a small arrow with a 44 px touch target; loading and playback recovery labels remain visible.

Inactive mobile sections remain rendered with opacity/visibility and inert, preserving masked rising text on entry and revisit. Hidden landscape sections are clipped to the content viewport so they cannot create empty scroll space. Reduced-motion preferences are still respected.

Validation: viewport layout and playback regressions, TypeScript, native-video touch/menu/revisit checks and local browser layout/typography comparisons. Both narrative sections contain four masked lines; copy does not wrap unexpectedly, and phone type-to-wordmark ratios are invariant. Desktop/tablet DOM bounds and every sampled media-frame placement match the previous release. Real-device Safari/Android testing has not been performed.
