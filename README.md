...
전환 중 영상 인물에는 원본 영상 픽셀을 기준으로 암부 톤 보정과 제한적인 미세 대비 보정을 적용합니다. 대기 화면의 원본 PNG에는 이 영상 보정을 적용하지 않습니다. 단색에 가까운 암부만 약하게 평활화하고, 강한 실루엣 경계는 선명도 보정에서 제외해 밝은 테두리를 억제합니다. 인물 마스크와 수증기 마스크는 보정 전 픽셀에서 계산하므로 로고가 인물 안으로 비치거나 수증기가 얼굴을 덮지 않습니다. 웹의 하단 명암 그라데이션은 WebGL에서 보정 후 합성하고 마지막에 1단계 미만의 고정 디더링을 적용해 8비트 출력의 계단 현상을 줄입니다. CSS 그라데이션은 WebGL을 사용할 수 없는 경우에 유지합니다. 4K 파일에도 이미 뭉개진 질감과 어두운 영역이 있어, 이 보정이 원본에 없는 세부를 복원하지는 않습니다.

화질을 위해 대기 장면은 생성 영상의 정지 프레임 대신 첨부 원본 `back.png`(1673×1190), `profile.png`(1674×1190), `front.png`(1906×1356)를 직접 사용합니다. 옆모습은 1442×1026 참고 이미지보다 큰 기존 1674×1190 원본을 유지합니다. 4K 영상의 중앙 아트워크 영역과 같은 좌표로 매핑하고, 장면 도착 후 0.32초에 걸쳐 원본으로 연결합니다. 다음 재생 시 0.18초에 걸쳐 영상으로 돌아갑니다. 배경·인물 마스크는 같은 원본과 같은 전환 비율을 공유합니다. 호흡·수증기·UI는 계속 동작하며 영상 재생 속도와 정지 시점은 그대로입니다.

PNG는 추가 압축·AI 재생성·가짜 해상도 확대 없이 사용합니다. 브라우저가 원본을 준비하지 못하면 기존 4K 영상으로 표시하며, 늦게 로드되어도 현재 대기 화면을 자동 갱신합니다. 이미지와 GPU 텍스처는 캐시하고 종료 시 해제합니다. 원본 스틸과 생성 영상의 외형이 완전히 동일하지 않으므로 짧은 혼합 전환을 사용하며, 움직이는 구간의 디테일은 기존 4K 영상 품질에 한정됩니다.
