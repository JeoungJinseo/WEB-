...
class NativeFilm {
  ...
  candidates(from,to){
    ...
    if(this.qualityPreference==='ultra'&&!this.hevcUnsupported)choices.unshift({key:direction,
  ...
  prefetch(){
    ...
    [this.index===2?1:this.index+1].forEach(i=>{
      ...
      if((!otherEntry||otherEntry.done)&&other?.getAttribute('src')){
        other.pause();other.removeAttribute('src');other.load();this.warming.delete(otherSlot);
      }
  ...
  async prepare(v,choice){
    const url=new URL(choice.url,location.href).href;
  ...
  destroy(){this.disposed=true;this.epoch++;this.stopWatch();clearTimeout(this.nextTask);clearTimeout(this.prefetchTimer);this.listeners.forEach(f=>f());
    this.slots.forEach(v=>{v.pause();v.removeAttribute('src');v.load();});this.warming?.clear();}
}
