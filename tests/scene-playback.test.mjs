...
async function setup({reduce=false,blocked=false,error=false}={}){
 ...
 class Video extends EventTarget {
  src='';readyState=0;duration=14.333;paused=true;seeking=false;time=0;plays=0;block=blocked;seeks=[];
 ...
 async function run(ms){for(let elapsed=0;elapsed<ms;elapsed+=20){now+=20;if(!video.paused){video.time+=.02;video.dispatchEvent(new Event('timeupdate'))}const pending=[...frames.values()];frames.clear();for(const fn of pending)fn(now);await flush()}}
...
}
{
 ...
 await h.run(5000);assert.equal(h.root.dataset.mode,'idle');assert.equal(h.video.currentTime,INTRO_END);assert.equal(h.video.paused,true);
 ...
 await h.run(5000);assert.equal(h.video.currentTime,SCENE_STOPS[1]);assert.equal(h.root.dataset.mode,'idle');assert.equal(h.video.plays,initialPlays+1,'extra wheel input cannot queue another scene');assert.ok(h.scenes.includes('profile'));
 await h.run(2000);assert.equal(h.video.currentTime,SCENE_STOPS[1],'holds without continued scrolling');
 h.wheel(100);await h.run(6000);assert.equal(h.video.currentTime,SCENE_STOPS[2]);assert.ok(h.scenes.includes('front'));
 h.wheel(-100);await h.run(6200);assert.equal(h.video.currentTime,SCENE_STOPS[1],'upward wheel rewinds one scene');
 h.film.replayIntro();await flush();assert.equal(h.video.paused,false);assert.equal(h.video.currentTime,0);await h.run(5000);assert.equal(h.video.currentTime,INTRO_END);
 h.emit('touchstart',{touches:[{clientY:600,clientX:100}]});h.emit('touchend',{changedTouches:[{clientY:450,clientX:105}]});await h.run(5000);assert.equal(h.video.currentTime,SCENE_STOPS[1],'one upward swipe advances');
 h.emit('keydown',{key:'PageDown',repeat:false});await h.run(6000);assert.equal(h.video.currentTime,SCENE_STOPS[2],'keyboard advances');
 h.film.dispose();assert.equal(h.video.paused,true);assert.equal(h.frames.size,0);
}
{
 const h=await setup({blocked:true});assert.equal(h.root.dataset.mode,'blocked');h.video.block=false;h.film.resume();await flush();await h.run(5000);assert.equal(h.root.dataset.mode,'idle');assert.equal(h.video.currentTime,INTRO_END);h.film.dispose();
}
{
